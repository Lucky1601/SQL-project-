...
add CONSTRAINT branch_bid_pk PRIMARY KEY(branch_no);
...
alter table account
add CONSTRAINT acnumber_pk primary key (acnumber);
alter table account 
add  CONSTRAINT account_custid_fk FOREIGN KEY(custid) REFERENCES customer(custid);
alter table account
add CONSTRAINT account_bid_fk FOREIGN KEY(bid) REFERENCES branch_mstr(branch_no);
...
add constraint emp_no_pk primary key(emp_no);
alter table employee
add constraint branch_no_fk foreign key(emp_no) references branch_mstr(branch_no);

insert into branch_mstr values
(1,'Delhi'),
(2,'Mumbai');
insert into customer values
(1,'Ramesh','Chandra','Sharma','Services','1976-12-06'),
(2,'Avinash','Sunder','Minha','Buisness','1974-10-16');
...
# Question 3 -Select unique occupation from customer table
select distinct(occupation)
from customer;

# Question 4 -Sort accounts according to current balance 
select *
from account
order by curbal;

# question 5 -Find the Date of Birth of customer name ‘Ramesh'
select dob
from customer
where fname='ramesh';

# Question 6 -Add column city to branch table
alter table branch_mstr
add column city char(20);

# Question 7 -Update the mname and lname of employee ‘Bella’ and set to ‘Karan’, ‘Singh’
update employee
set mname='Karan' , lname='Singh'
where fanme='bella';
## I accidently put the wrong cloumn name fname to fanme now I am changing it in next line
alter table employee 
rename column fanme to fname;

# Question 8 - Select accounts opened between '2012-07-01' AND '2013-01-01'
select *
from account
where opndt between '2012-07-01' AND '2013-01-01';

# Question 9 - List the names of customers having ‘a’ as the second letter in their names 
select fname 
from customer
where fname like '_a%';

# Question 10 - Find the lowest balance from customer and account table
select acnumber, min(curbal)
from account;

# Question 11 - Give the count of customer for each occupation
select fname, count(custid),occupation
from customer
group by occupation;

# Question 12 - Write a query to find the name (first_name, last_name) of the employees who are managers.
select fname,lname 
from employee
where desig = 'manager';

# Question 13 - List name of all employees whose name ends with a
select fname
from employee
where fname like '%a';

# Question 14 - Select the details of the employee who work either for department ‘loan’ or ‘credit’
select * 
from employee
where dept = 'loan' or 'credit';

# Question 16 - Write a query to display the customer’s number, customer’s firstname, branch id and balance amount for people using JOIN.
SELECT fname,bid,curbal
FROM customer ct  JOIN branch_mstr br ON ac.custid=br.custid join account ac
ORDER BY account_number;

# Question 17. Create a virtual table to store the customers who are having the accounts in the same city as they live



# Question 18. A. Create a transaction table with following details 
...
bid int,
amount int,
type_of_transaction char(10),
dot date,
constraint chk1 check (type_of_transaction = 'Withdraw' or type_of_transaction = 'Deposit'),
constraint account_custid1_fk foreign key (custid) references customer (custid),
constraint account_acnumber1_fk foreign key (acnumber) references account (acnumber),
constraint account_bid_1fk foreign key (bid) references branch_mstr (branch_no));
...
set curr_balance = (
select curbal from account a where a.acnumber=acnumber);
if new.type='Withdraw' then
update account set a.curbal = curr_balance - curr_balance where a.acnumber=acnumber;
elseif new.type='Deposit' then
update accounts set curbal = curr_balance + curr_balance where a.acnumber=acnumber;
end if;
end $$



# Question 19. Write a query to display the details of customer with second highest balance .

SELECT c.custid,Fname,ltname,curbal 
FROM customer c join account a
on c.custid = a.custid
WHERE curbal limit 1 offset 1;
...
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(909,'H.NO.16, Sector-4, 14th Cross','Near BDA Complex, HSR 
Layout','Bangalore','Karnataka',560172,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(910,'H.NO.15, Sector-5, 7th Main','HSR Layout','Bangalore','Karnataka',560172,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(911,'Flat-8, 2689/29','Tuglakkabad extn','New Delhi','Delhi',110019,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(912,'Flat-10, 2689/27','Nizamuddin','New Delhi','Delhi',110012,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(914,'H.NO.20,Heritage Apartments','Udayagiri','Mysore','Karnataka',570019,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(915,'2686/23, Surya Apartments','Badarpur','New Delhi','Delhi',110013,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(916,'No.188,1st Main,3rd 
Cross','Subramanyanagar','Bangalore','Karnataka',560021,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(917,'No.380,Sri Sai Complex','Ulsoor','Mysore','Karnataka',562109,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(918,'# 55/1,5th Cross,S.P.Road','Hosur','Hosur','Tamilnadu',635235,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(919,'43/3,Mukesh Complex','Madanayakanapalli','Hyderabad','Andhra 
Pradesh',517247,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(920,'824/1,Krishna Complex','Bagalpur Circle','Krishnagiri','Tamilnadu',635109,'India');
Insert into ADDRESS
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(921,'162/S2,Margosa Road,','Malleswaram','Bangalore','Karnataka',560003,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(922,'No.65,9th Main,Begur Road','Bommanhalli','Mysore','Karnataka',570019,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(923,'#52,N.K.Building','Bijai','Mangalore','Karnataka',575002,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(924,'1160,Rainbow Apartments','Parangipalyam','Chittoor','Andhra 
Pradesh',517337,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(925,'No.1610,Kuppu Swamy Complex','SVR Layout','Salem','Tamilnadu',635203,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(926,'#140,4th Main,6th Cross','Manikonda IT Park','Hyderabad','Andhra 
Pradesh',517252,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(927,'#155, 2nd Main Channal 
Road','Saraswethipuram','Dharmapuri','Tamilnadu',635897,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(928,'#412, 100ft Road,4th Block','Koramangala','Bangalore','Karnataka',560034,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(913,'No. 354, 3/5/343','1st Main, 2nd Cross, Jayanagar 4th 
Blk','Bangalore','Karnataka',560005,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(1000,'Anand Engineering, Plot No. 66, Road No. 15/17','MIDC, Andheri East, 
','Mumbai','Maharashtra',400093,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(1001,'59-A, Road INT 1, ','Mulgaon, Andheri 
East,','Mumbai','Maharashtra',400047,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(1002,'Acme Plaza, Andheri - Kurla Rd, ','Vijay Nagar Colony, J B Nagar, Andheri 
East,','Mumbai','Maharashtra',400053,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(1003,'#41, Main Road,4th Block','Opp. Forum Mall, 
Koramangala','Bangalore','Karnataka',560034,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(1004,'Collector Chawl, Kondivita Road, ','J B Nagar, Andheri 
East','Mumbai','Maharashtra',400059,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(1005,'Jeevan Sandhya CHS, Bharat Ark, 4, Veera Desa#6','Mhada Colony, Azad Nagar, 
Andheri West','Mumbai','Maharashtra',400053,'India');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(930,'8340','Pilgrim Lane','Fargo','ND',58012,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(931,'938','SE. 53rd Street','Scarsdale','NY',10583,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(932,'7834','Theatre St.','Brooklyn','NY',11201,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(933,'777','Brockton Avenue','Abington','MA',2351,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(934,'30','Memorial Drive','Avon','MA',2322,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(935,'250','Hartford Avenue','Bellingham','MA',2019,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(936,'141','Washington Ave','Albany','NY',12205,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(937,'13858','Rt 31 Brookfield St','W. Alibio','NY',14411,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(938,'2055','Niagara Falls Blvd','Amherst','NY',14228,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(939,'2011','Niagara Falls Blvd','Amherst','NY',14228,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(940,'315','Foxon Blvd, New','Haven','CT',6513,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(941,'164','Danbury Rd, New','Milford','CT',6776,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(942,'3164','Berlin Turnpike','Newington','CT',6111,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(943,'474','Boston Post Road','North Windham','CT',6256,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(944,'650','Main Ave','Norwalk','CT',6851,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(945,'1600','Montclair Rd','Birmingham','AL',35210,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(946,'5919','Trussville Crossings Pkwy','Birmingham','AL',35235,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(947,'5360','Southwestern Blvd','Hamburg','NY',14075,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(948,'103','North Caroline St','Herkimer','NY',13350,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(949,'1000','State Route 36','Hornell','NY',14843,'USA');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(950,'1 Holland Grove Road','#23-34 Beachview Apts','Bukit 
Timah',null,278790,'Singapore');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(951,'16, Sandilands Road','#12-45 Changi Towers','Singapore',null,546080,'Singapore');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(952,'Blk 35 Mandalay Road','# 13–37 Mandalay Towers 
','Singapore',null,308215,'Singapore');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(953,'10 Eunos Road 8 ','Singapore Post Centre ','Singapore',null,408600,'Singapore');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(954,'Bruderer 65 ','Loyang Way','Singapore',null,508755,'Singapore');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(955,'277 Orchard Road','Tampines','Singapore',null,238858,'Singapore');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(956,'75 Kg Sg Ramal Luar','Kajang ','Selangor',null,43000,'Malaysia');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(957,'Apt #23, Putra Towers','45 Jalan Tun Ismail ','Kuala Lumpur',null,50480,'Malaysia');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(958,'Apt #24, Putra Towers','45 Jalan Tun Ismail ','Kuala Lumpur',null,50480,'Malaysia');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(959,'476 Jalan Tun Razak','77A Jalan Sultan Sulaiman, ','Kuala 
Terengganu',null,20000,'Malaysia');
Insert into ADDRESS 
(ADDRESS_ID,ADDRESS_LINE1,ADDRESS_LINE2,CITY,STATE,PINCODE,COUNTRY) values 
(960,'205 Shanthi Villa','Silkhouse Street','Kandy',null,20000,'Sri Lanka');
...
# Question 1 - Display the product details as per the following criteria
...
end as new_product_price from product order by product_class_code desc;



# Question 2. List the product description, class description and price of all products which are shipped
select product_desc,product_class_desc ,product_price,order_status from product p join product_class pc
on p.PRODUCT_CLASS_CODE = pc.PRODUCT_CLASS_CODE join order_items o 
on o.PRODUCT_ID = p.PRODUCT_ID join order_header oh 
on oh.ORDER_ID = o.order_id where ORDER_STATUS = 'Shipped';



## Question 3 - . Show inventory status of products as below as per their available quantity:
...
## Question 4 - List customers from outside Karnataka who haven’t bought any toys or books
select distinct concat(customer_fname," ",customer_lname) as customer_name,CUSTOMER_USERNAME,  State ,product_class_desc  
from product_class pc join product p on p.product_class_code = pc.product_class_code join 
order_items o on
o.product_id = p.product_id join order_header oh on 
o.order_id = oh.order_id join online_customer oc on 
oc.customer_id = oh.customer_id join address a on 
a.address_id = oc.address_id where state != 'Karnataka' and (product_class_desc != 'Toys' and product_class_desc != 'Books');
